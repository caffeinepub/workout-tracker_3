...
import { ArrowLeft, Plus, Loader2, Dumbbell, Calendar, BarChart2 } from 'lucide-react';
...
import { ExerciseId, WeightUnit } from '../backend';
...
export default function ExerciseLogView({ exerciseId, exerciseName, onBack }: ExerciseLogViewProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!sets || !reps || !weight || isNaN(setsNum) || isNaN(repsNum) || isNaN(weightNum)) {
      toast.error('Please fill in all fields with valid numbers.');
      return;
    }

    // Convert to lbs (base unit) before storing
    const weightInLbs = Math.round(convertWeight(weightNum, weightUnit, WeightUnit.lbs));
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-1.5">
                <Label htmlFor="sets" className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  Sets
                </Label>
                <Input
                  id="sets"
              ...
              <div className="space-y-1.5">
                <Label htmlFor="reps" className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  Reps
                </Label>
                <Input
                  id="reps"
              ...
              <div className="space-y-1.5">
                <Label htmlFor="weight" className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  Weight ({weightUnit})
                </Label>
                <Input
                  id="weight"
        ...
        {!isLoading && !isError && logs && logs.length > 0 && (
          <div className="space-y-3">
            {logs.map((log) => {
              ...
              const volume = Number(log.sets) * Number(log.reps) * displayedWeight;
              return (
