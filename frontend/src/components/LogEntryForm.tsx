...
import { WorkoutLog, LogExercise } from '../utils/localStorageLogs';
import { useUpdateLocalLog, useMarkLogComplete } from '../hooks/useLocalLogs';
...
export default function LogEntryForm({ log, onBack }: Props) {
  ...
  const updateActual = (
    index: number,
    field: keyof Pick<LogExercise, 'actualSets' | 'actualReps' | 'actualWeight' | 'actualTime'>,
    value: string
  ) => {
    setExercises((prev) => {
      const updated = [...prev];
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div className="bg-muted/50 rounded-lg p-2 text-center">
                      ...
                      <p className="font-bold">{ex.plannedWeight > 0 ? `${ex.plannedWeight} lbs` : '—'}</p>
                    </div>
                    <div className="bg-muted/50 rounded-lg p-2 text-center">
                      <p className="text-xs text-muted-foreground">Time</p>
                      <p className="font-bold">{ex.plannedTime > 0 ? `${ex.plannedTime}m` : '—'}</p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <Input
                        ...
                        value={ex.actualSets ?? ''}
                        onChange={(e) => updateActual(idx, 'actualSets', e.target.value)}
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        value={ex.actualReps ?? ''}
                        onChange={(e) => updateActual(idx, 'actualReps', e.target.value)}
                        disabled={isCompleted}
                        className="h-8 text-sm"
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        type="number"
                        min={0}
                        placeholder={ex.plannedWeight > 0 ? String(ex.plannedWeight) : '0'}
                        value={ex.actualWeight ?? ''}
                        onChange={(e) => updateActual(idx, 'actualWeight', e.target.value)}
                        disabled={isCompleted}
                        className="h-8 text-sm"
                      />
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">Time (m)</p>
                      <Input
                        type="number"
                        min={0}
                        placeholder={ex.plannedTime > 0 ? String(ex.plannedTime) : '0'}
                        value={ex.actualTime ?? ''}
                        onChange={(e) => updateActual(idx, 'actualTime', e.target.value)}
                        disabled={isCompleted}
                        className="h-8 text-sm"
                      />
                    </div>
