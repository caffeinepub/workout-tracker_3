...
import { WorkoutSession, WeightUnit } from '../backend';
...
export default function ProgressionChart({ workouts, exerciseName }: ProgressionChartProps) {
  ...
  const chartData = workouts.map((workout) => {
    ...
    return {
      ...
      weight: parseFloat(displayedWeight.toFixed(1)),
          ...
          <TabsContent value="volume" className="space-y-4">
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={chartData}>
                ...
                <Legend />
                <Bar dataKey="volume" fill="oklch(var(--chart-1))" name={`Total Volume (${weightUnit})`} radius={[8, 8, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </TabsContent>
          ...
          <TabsContent value="weight" className="space-y-4">
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={chartData}>
                ...
                <Tooltip content={<CustomTooltip />} />
                <Legend />
                <Line type="monotone" dataKey="weight" stroke="oklch(var(--chart-2))" strokeWidth={3} name={`Weight (${weightUnit})`} dot={{ r: 5 }} />
              </LineChart>
            </ResponsiveContainer>
          </TabsContent>
